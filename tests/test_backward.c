...
static int cmp_dev(RefFile *r, const char *gradname, const float *dptr, long n) {
  if (n > hcap) { hcap = n; hbuf = (float *)realloc(hbuf, n * 4); }
  CK(cudaMemcpy(hbuf, dptr, n * 4, cudaMemcpyDeviceToHost));
  const float *ref = ref_f32(r, gradname);
  ...
  printf("  %-18s n=%-7ld maxabs=%.2e maxrel=%.2e bad=%ld -> %s\n",
         gradname, n, maxabs, maxrel, bad, bad ? "FAIL" : "ok");
  return bad == 0;
}
...
int main(int argc, char **argv) {
  ...
  printf("backward:\n");
  int ok = 1;
  ok &= cmp_dev(r, "wte.grad", m->g.wte, V * d);
  ok &= cmp_dev(r, "wpe.grad", m->g.wpe, S * d);
  char nm[64];
  for (int l = 0; l < cfg.n_layer; l++) {
    LayerW *g = &m->g.layer[l];
#define C(field, suffix, cnt) do { snprintf(nm, sizeof(nm), "blk%d." suffix ".grad", l); ok &= cmp_dev(r, nm, g->field, cnt); } while (0)
...
#undef C
  ...
  ok &= cmp_dev(r, "lnf.w.grad", m->g.lnf_w, d);
  ok &= cmp_dev(r, "lnf.b.grad", m->g.lnf_b, d);
