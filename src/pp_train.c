...
static float pp_step(Comms *c, Model *m, int M, int P, int stage, int nslots, long Rd,
  ...
  for (int i = 0; i < remaining; i++) {                      // steady 1F1B
    model_set_slot(m, ff % nslots); model_set_input(m, mi + (long)ff * R, mt + (long)ff * R);
    float l = model_forward_pp(m, xout); if (last) lsum += l; ff++;
    if (!last) comms_sendrecv_bf16(c, xout, Rd, stage + 1, m->pp_dxout, Rd, stage + 1);
    model_set_slot(m, bb % nslots); model_backward_pp(m, dxin, inv_mb); bb++;
    if (i == remaining - 1) { if (!first) comms_send_bf16(c, dxin, Rd, stage - 1); }
    else if (!first) comms_sendrecv_bf16(c, dxin, Rd, stage - 1, m->pp_xin, Rd, stage - 1);
  }
...
int main(int argc, char **argv) {
  ...
  if (stage == 0)
    printf("PP P=%d  L=%d(%d/stage) d=%d h=%d seq=%d mbatch=%d M=%d\n", P, L, nl, d, H, seq, Bm, M);
  ...
  if (stage == 0) {
    ...
    printf("P=%d M=%d: step %.2f ms  ideal %.2f ms  bubble %.1f%% (theory=%.1f%%)\n",
